...
public class CustomItem {
    ...
    public static void init() {
        ...
        foods.forEach(
                (key, value) -> Food.registerFood(new FoodNormal(value, 4), InnerCoreServer.plugin).addRelative(key));
    ...
    public static void addCreative(int id, int count, int data, NativeItemInstanceExtra extra) {
    ...
    private static final Consumer<ItemCreative> func = item -> Item.addCreativeItem(407,
            ItemUtils.get(item.id, item.count, item.data, item.extra));
