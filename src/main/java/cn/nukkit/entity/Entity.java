...
public abstract class Entity extends Location implements Metadatable {
    ...
    public void recalculateBoundingBox(boolean send) {
        ...
        if (send) {
            sendData((Player[]) this.hasSpawned.values().toArray(Player.EMPTY_ARRAY), new EntityMetadata().put(bbH).put(bbW));
        }
    }
...
//                    Server.broadcastPacket(this.hasSpawned.values(), pk);
    ...
    public void respawnToAll() {
        Player[] players = (Player[]) this.hasSpawned.values().toArray(Player.EMPTY_ARRAY);
