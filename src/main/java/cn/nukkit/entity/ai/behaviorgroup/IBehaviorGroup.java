 ...
 * 它由若干个（核心）行为{@link IBehavior}、控制器{@link IController}、传感器以及一个寻路器{@link IRouteFinder}和记忆存储器{@link IMemoryStorage}组成br>
 * 注：核心行为指的是不会被行为优先级影响的行为，其激活状态只取决于其自身的评估器br>
 * <p>
 * A Behavior Group is a basic, self-contained unit of AI<br>
 * that consists of several (core) Behaviors{@link IBehavior}, Controllers{@link IController}, Sensors and a Pathfinder{@link IRouteFinder} and memory {@link IMemoryStorage} are composed<br>
