    ...
    public boolean search() {
        ...
        if (EntityAI.checkDebugOption(EntityAI.DebugOption.ROUTE)) {
            if (System.currentTimeMillis() - lastRouteParticleSpawn > EntityAI.getRouteParticleSpawnInterval()) {
                findingPath.forEach(node -> this.entity.level.addParticle(new BlockForceFieldParticle(node.getVector3()), (Player[]) Server.getInstance().getOnlinePlayers().values().toArray(Player.EMPTY_ARRAY)));
